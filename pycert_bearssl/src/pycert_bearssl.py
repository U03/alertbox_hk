...
def download(port, cert_var, cert_length_var, output, use_store, keep_dupes, domain):
    ...
    for d in domain:
        ...
        click.echo('Retrieved certificate for {0}'.format(d), err=True)
    ...
    try:
      ...
      click.echo(f'Recieved error when converting certificate to header: {E}', err=True)
      exit(1)
...
def convert(cert_var, cert_length_var, output, use_store, keep_dupes, no_search, cert):
    ...
    for c in cert:
        ...
        if cert_parsed is None:
          click.echo('Failed to load certificate {0}'.format(c.name), err=True)
        else:
          click.echo('Loaded certificate {0}'.format(c.name), err=True)
          ...
          if no_search and cert_parsed.get_subject() != cert_parsed.get_issuer():
            click.echo(f'Warning: certificate {c.name} is an end entity certificate (not a CA). '
                        'SSLClient may fail to validate if the server certificate changes.', err=True)
    ...
    if no_search:
      ...
      for i, c in enumerate(cert_objs):
        ...
        if cn_hash not in cert_dict:
          click.echo('Could not find a root certificate for {0}'.format(cert[i].name), err=True)
        else:
    ...
    try:
      ...
      click.echo(f'Wrote {written} trust anchors to {output.name}', err=True)
    except Exception as E:
      click.echo(f'Recieved error when converting certificate to header: {E}', err=True)
      exit(1)
